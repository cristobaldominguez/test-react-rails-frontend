...
import { userLocalStorageKey, apiUrl } from '../../config.js'
    ...
    <Disclosure as='nav' className='bg-white dark:bg-neutral-950 shadow'>
        ...
        <div className='flex h-16 justify-between'>
          <div className='flex px-2 lg:px-0'>
            ...
            <div className='hidden lg:ml-6 lg:flex lg:space-x-8'>
              ...
              <NavLink to='/calendar' className={({ isActive }) => isActive ? `${css.desktop_menu} ${css.active}` : `${css.desktop_menu} ${css.default}`}> Calendar </NavLink>
            </div>
          </div>
          ...
          {isAuthenticated && (
            <div className='hidden lg:ml-4 lg:flex lg:items-center'>
              <button type='button' className='relative flex-shrink-0 rounded-full bg-white dark:bg-neutral-800 p-1 text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2'>
              ...
              <Menu as='div' className='relative ml-4 flex-shrink-0'>
                ...
                <MenuItems transition className='absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white dark:bg-neutral-900 py-1 shadow-lg ring-1 ring-black ring-opacity-5 transition focus:outline-none data-[closed]:scale-95 data-[closed]:transform data-[closed]:opacity-0 data-[enter]:duration-100 data-[leave]:duration-75 data-[enter]:ease-out data-[leave]:ease-in'>
                  <MenuItem><a href='#' className='block px-4 py-2 text-sm text-gray-700 dark:text-white data-[focus]:bg-gray-100 dark:data-[focus]:bg-neutral-800'> Your Profile </a></MenuItem>
                  <MenuItem><a href='#' className='block px-4 py-2 text-sm text-gray-700 dark:text-white data-[focus]:bg-gray-100 dark:data-[focus]:bg-neutral-800'> Settings </a></MenuItem>
                  <MenuItem><a href='#' onClick={logoutHandler} className='block px-4 py-2 text-sm text-gray-700 dark:text-white data-[focus]:bg-gray-100 dark:data-[focus]:bg-neutral-800'> Sign out </a></MenuItem>
                </MenuItems>
              </Menu>
            </div>
          )}
      ...
      <DisclosurePanel className='lg:hidden'>
        <div className='space-y-1 pb-3 pt-2'>
          ...
          <NavLink to='/calendar' className={({ isActive }) => isActive ? `${css.mobile_menu} ${css.active}` : `${css.mobile_menu} ${css.default}`}> Calendar </NavLink>
        </div>
        {isAuthenticated && (
